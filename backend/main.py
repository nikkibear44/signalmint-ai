...
# ===========================
# Premium Portfolio Health (paid, x402)
# ===========================

@app.post("/x402/portfolio-health")
def portfolio_health_paid(data: PortfolioHealthRequest, request: Request):

    if not data.chain.strip() or not data.address.strip():
        return {"error": "Both chain and address are required."}

    resource_url = str(request.url)
    description = "AI-generated premium wallet health report with rebalancing suggestions and scam-token detection"
    ...
    result = portfolio_health_premium(data.chain, data.address)

    response = response_template("x402/portfolio-health")
    response.update(
        {
            "chain": data.chain,
